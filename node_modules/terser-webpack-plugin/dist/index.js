...
class TerserPlugin {
  ...
  async optimize(compiler, compilation, assets) {
    const assetNames = Object.keys(assets).filter(assetName => compiler.webpack.ModuleFilenameHelpers.matchObject.bind( // eslint-disable-next-line no-undefined
      ...
      scheduledTasks.push(limit(async () => {
        ...
        if (!output) {
          ...
          try {
            ...
            compilation.errors.push(TerserPlugin.buildError(error, name, inputSourceMap && TerserPlugin.isSourceMap(inputSourceMap) ? new _sourceMap.SourceMapConsumer(inputSourceMap) : null, compilation.requestShortener));
            return;
          }
          ...
          if (output.extractedComments && output.extractedComments.length > 0) {
            ...
            if (this.options.extractComments.banner !== false) {
              banner = this.options.extractComments.banner || `For license information please see ${_path.default.relative(_path.default.dirname(name), output.commentsFilename).replace(/\\/g, '/')}`;
