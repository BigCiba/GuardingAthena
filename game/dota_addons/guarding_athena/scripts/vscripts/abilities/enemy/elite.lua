...
function modifier_elite:OnCreated(params)
	...
	if IsServer() then
		...
		hParent:SetMinimumGoldBounty(hParent:GetGoldBounty() * (Spawner.difficulty / 2)  * Spawner.unitFactor.eliteFactor ^ Spawner.gameRound)
		hParent:SetMaximumGoldBounty(hParent:GetGoldBounty() * (Spawner.difficulty / 2)  * Spawner.unitFactor.eliteFactor ^ Spawner.gameRound)
		hParent:SetBaseDamageMin(hParent:GetBaseDamageMin() * (Spawner.difficulty / 2 )  * Spawner.unitFactor.eliteFactor ^ Spawner.gameRound)
		hParent:SetBaseDamageMax(hParent:GetBaseDamageMax() * (Spawner.difficulty / 2 )  * Spawner.unitFactor.eliteFactor ^ Spawner.gameRound)
		hParent:SetBaseMaxHealth(hParent:GetBaseMaxHealth() * (Spawner.difficulty / 2 )  * Spawner.unitFactor.eliteFactor ^ Spawner.gameRound)
