LinkLuaModifier("modifier_item_water_gun", "abilities/items/drops/item_water_gun.lua", LUA_MODIFIER_MOTION_NONE)
-- Abilities
if item_water_gun == nil then
...
function item_water_gun:OnProjectileHit(hTarget, vLocation)
	hTarget:AddNewModifier(self:GetCaster(), self, "modifier_item_water_gun", {duration = self:GetDuration()})
end
---------------------------------------------------------------------
-- Modifiers
if modifier_item_water_gun == nil then
	modifier_item_water_gun = class({}, nil, ModifierDebuff)
end
...
function modifier_item_water_gun:GetModifierHealthRegenPercentage(params)
