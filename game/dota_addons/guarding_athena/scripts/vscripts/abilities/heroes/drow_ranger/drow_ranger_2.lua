LinkLuaModifier("modifier_drow_ranger_2", "abilities/heroes/drow_ranger/drow_ranger_2.lua", LUA_MODIFIER_MOTION_NONE)
-- Abilities
if drow_ranger_2 == nil then
	drow_ranger_2 = class({})
end
function drow_ranger_2:OnSpellStart()
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_drow_ranger_2", {duration = 4})
end
function drow_ranger_2:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
...
if modifier_drow_ranger_2 == nil then
	modifier_drow_ranger_2 = class({}, nil, ModifierPositiveBuff)
end
function modifier_drow_ranger_2:OnCreated(params)
	self.count = self:GetAbilitySpecialValueFor("count")
	self.interval = self:GetAbilitySpecialValueFor("interval")
	if IsServer() then
	end
end
function modifier_drow_ranger_2:OnRefresh(params)
	if IsServer() then
	end
end
function modifier_drow_ranger_2:OnDestroy()
	if IsServer() then
	end
end
...
function modifier_drow_ranger_2:OnAttack(params)
	if IsServer() then
