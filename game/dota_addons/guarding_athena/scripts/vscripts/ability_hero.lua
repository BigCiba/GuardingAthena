...
function DemonHunter( keys )
	...
	if caster:HasModifier("modifier_demon_hunter_buff") == false then
		...
		if stackcount < soulCount and caster:HasModifier("modifier_demon_hunter_spell") == false then
			caster:SetModifierStackCount("modifier_demon_hunter_buff",caster,stackcount + 1)
			EmitSoundOn("Hero_Terrorblade.Attack.Rip", caster)
			caster:CalculateStatBonus()
		end
	end
...
function DemonHunterSpell( keys )
	...
	Timers:CreateTimer(10,function ()
		...
		if caster.soul_stack >= 100 then
			...
			caster:CalculateStatBonus()
			caster:RemoveModifierByName("modifier_demon_hunter_buff")
...
function SoulBlade( keys )
	...
	local damage = keys.Damage * caster:GetAgility() + keys.Basedamage
	local stackcount = caster:GetModifierStackCount("modifier_demon_hunter_buff", caster)
	local angle = 140
	local radius = 382
...
function SoulRing( keys )
	...
	local damageType = ability:GetAbilityDamageType()
	local fxIndex = CreateParticle( "particles/skills/mana_field.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster )
	...
	Timers:CreateTimer(15,function() ParticleManager:DestroyParticle(fxIndex,false) end)
	local time = 0
	Timers:CreateTimer(function ()
		...
		for k, v in pairs( targets ) do
		    if caster:HasModifier("modifier_demon_hunter_spell") then
		    	local stackcount = caster:GetModifierStackCount("modifier_demon_hunter_buff", caster)
		    	currentDamage = damage * (1 + stackcount * 0.02) * 0.1
		    else
		    	currentDamage = damage * 0.1
			end
...
function MagicBlade( keys )
	local caster = keys.caster
	local target = keys.target
	local ability = keys.ability
	local attack = keys.Attack
	local damage = keys.Damage * caster:GetAgility()
	local damageType = ability:GetAbilityDamageType()
	if target:HasModifier("modifier_magic_blade_debuff") then
		local debuffcount =  target:GetModifierStackCount("modifier_magic_blade_debuff",ability)
		ability:ApplyDataDrivenModifier(caster,target,"modifier_magic_blade_debuff",nil)
		target:SetModifierStackCount("modifier_magic_blade_debuff",ability,debuffcount + attack)
