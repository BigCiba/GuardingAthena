...
function Ultrasonic( t )
    ...
    local unitGroup = GetUnitsInLine(caster,ability,caster_location,point,450)
    for k, v in pairs( unitGroup ) do
        CauseDamage(caster, v, damage, damageType, ability)
        for i=1,5 do
            if v:GetAbilityByIndex(i-1) then
                local ability = v:GetAbilityByIndex(i-1)
                if ability:IsCooldownReady() and not ability:IsToggle() and not ability:GetCooldownTime() == 0 then
    ...
    Timers:CreateTimer(duration,function ()
        if ability.illusion_1:IsNull() == false then
            if ability.illusion_1:IsAlive() then
                ...
                Heal(caster,caster:GetMaxHealth(),false)
